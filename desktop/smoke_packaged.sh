...
# --- (d2) scanned-page OCR works in the bundle (vendored tesseract) -----------
# A 1-page image-only PDF (rendered glyphs, no text layer) is generated by
# desktop/make_smoke_scan.py with the BUILD machine's pipeline venv; if the
# vendored tesseract is missing or broken, extraction yields no text and the
# doc terminally parks in failed/needs_review instead of ready. Runs BEFORE the
# review-job step: the cancelled job's activity marks defer ingest ~10-20s.
scan_pdf="$SCRATCH_DIR/smoke_scan.pdf"
"pipeline/.venv/bin/python" desktop/make_smoke_scan.py "$scan_pdf" \
  || fail "make_smoke_scan.py failed (pipeline venv / PyMuPDF)"
...
while [[ $SECONDS -lt $deadline ]]; do
  sstat="$(curl -s "$BASE/kb/documents?matter=$matter_slug" | jq -r --arg id "$sid" '.documents[] | select((.id|tostring)==$id) | .status' 2>/dev/null)"
  [[ "$sstat" == "ready" ]] && break
  if [[ "$sstat" == "failed" || "$sstat" == "needs_review" ]]; then
    fail "scanned PDF ingest ended $sstat — vendored tesseract broken?"
  fi
  sleep 1
done
[[ "$sstat" == "ready" ]] || fail "scanned PDF never reached ready (last: '$sstat') — vendored tesseract broken?"
echo "==> (d2) scanned-page OCR -> ready (vendored tesseract works in the bundle)"
